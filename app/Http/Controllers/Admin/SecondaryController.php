...
class SecondaryController extends Controller
{
    ...
    public function get_secondary_defined_block(Request $request)
    {
        ...
        $get_property = Property::where('gis_id', $gis_id)
            ...
            ->whereIn('temp_gis_id_status', [0, 2])
            ->where('created_by', Auth::user()->id)
            ->first();
        // ->where('created_by',Auth::user()->id)
        if ($get_property) {
            if ($get_property->residential_sub_type == 10 || $get_property->residential_sub_type == 12 || $get_property->residential_sub_type == 17 || $get_property->residential_sub_type == 18 || $get_property->plot_land_type == 14) {
                $secondary_blade_slug = Category::whereIn('id', [10, 12, 14, 17, 18])->value('secondary_blade_slug');
    ...
    public function createTowers(Request $request)
    {
        ...
        if (isset($request->block_id)) {
            ...
            if (isset($request->tower_name0)) {
                ...
                $rules['no_of_towers0'] = 'required|integer|min:1';
                $error_messages['no_of_towers0.required'] = 'No of Tower/Unit field required.';
                $error_messages['no_of_towers0.min'] = 'No of Tower/Unit field required.';
            }
        }
        $validator = Validator::make(
            $request->all(),

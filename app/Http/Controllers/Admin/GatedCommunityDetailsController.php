...
use App\Models\{ProjectStatus, UnderConstruction, SecondaryFeatures, SecondaryFeaturesOptions, FloorType, Unit, Property, Category, FloorUnitCategory, Block, Tower, PropertyFloorMap, FloorUnitMap, Ammenitity, PropertyAmenity, Compliances, ProjectRepository, BlockTowerRepository, GeoID, PropertyAmenityAmenityOption, PriceTrend, ProjectStatusLog, TowerLog};
use Validator;
...
class GatedCommunityDetailsController extends Controller
{
    ...
    {
        $property = Property::where('created_by', Auth::user()->id)
            ->where('id', $id)
            ->first();
    ...
    public function amenities(Request $request)
    {
        ...
        $propertyAmenities = PropertyAmenity::with('children')
            ->where('property_category_id', config('constants.GATED_COMMUNITY_APARTMENT'))
            ->get();
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($property) {
            if ($property->residential_sub_type == 10 || $property->residential_sub_type == 12 || $property->residential_sub_type == 17 || $property->plot_land_type == 14) {
                $general_detail_blade_slug = Category::whereIn('id', [10, 12, 14, 17])->value('secondary_blade_slug');

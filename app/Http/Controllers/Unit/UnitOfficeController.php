...
class UnitOfficeController extends Controller
{
    ...
    public function storeCommOfcPropertyDetails(Request $request)
    {
        $validator = Validator::make(
            ...
            [
                ...
                'min_no_of_seats' => ['required_if:office_type,==,"83"'],
                'max_no_of_seats' => ['required_if:office_type,==,"83"'],
                'no_of_cabins' => ['required_if:office_type,==,"83"'],
                'no_of_meeting_rooms' => ['required_if:office_type,==,"83"'],
                'pantry' => ['required_if:office_type,==,"83"'],
                'conference_room' => ['required_if:office_type,==,"83"'],
                'reception_area' => ['required_if:office_type,==,"83"'],
                'furnishing' => ['required_if:office_type,==,"83"'],
                'central_air_conditions' => ['required_if:office_type,==,"83"'],
                'oxygen_dust' => ['required_if:office_type,==,"83"'],
                'ups' => ['required_if:office_type,==,"83"'],
                // 'fire_safety_masure' => ['required_if:office_type,==,"83"'],
                'staircase' => ['required_if:office_type,==,"83"'],
                'lift' => ['required_if:office_type,==,"83"'],
                // 'owners_preference' => ['required'],
            ],
            [
                'carpet_area.required_without_all' => 'Any one field is required',
                'min_no_of_seats.required_if' => ['Minimum no. of seats required'],
                'max_no_of_seats.required_if' => ['Maximum no. of seats required'],
                'no_of_cabins.required_if' => ['No. of cabins required'],
                'no_of_cabins.required_if' => ['No. of cabins required'],
                'no_of_meeting_rooms.required_if' => ['No. of meeting rooms required'],
                'pantry.required_if' => ['Pantry required'],
                'conference_room.required_if' => ['Conference room required'],
                'reception_area.required_if' => ['Reception area required'],
                'furnishing.required_if' => ['Furnishing required'],
                'central_air_conditions.required_if' => ['Central air conditioning required'],
                'oxygen_dust.required_if' => ['Oxygen dust required'],
                'ups.required_if' => ['UPS required'],
                // 'fire_safety_masure.required_if' => ['Fire safety measure required'],
                'staircase.required_if' => ['Staircase required'],
                'lift.required_if' => ['Lift required'],
                // 'owners_preference.required_if' => ['Owners preference required'],
            ],
        );
        if ($validator->fails()) {
            if ($request->ajax()) {
        ...
        $save = SecondaryUnitLevelData::updateOrCreate(
            ...
            [
                ...
                'min_no_of_seats' => $request->office_type == '83' ? $request->min_no_of_seats : null,
                'max_no_of_seats' => $request->office_type == '83' ? $request->max_no_of_seats : null,
                'no_of_cabins' => $request->office_type == '83' ? $request->no_of_cabins : null,
                'no_of_meeting_rooms' => $request->office_type == '83' ? $request->no_of_meeting_rooms : null,
                'staircase' => $request->office_type == '83' ? $request->staircase : null,
                'owners_preference' => $request->owners_preference,
                'pantry' => $request->office_type == '83' ? $request->pantry : null,
                'property_facing' => $request->property_facing,
                'conference_room' => $request->office_type == '83' ? $request->conference_room : null,
                'reception_area' => $request->office_type == '83' ? $request->reception_area : null,
                //'furnishing' => $request->office_type == "83" ? $request->furnishing : null,
                'furnishing' => $request->office_type != null ? $request->furnishing : null,
                'central_air_conditions' => $request->office_type == '83' ? $request->central_air_conditions : null,
                'oxygen_dust' => $request->office_type == '83' ? $request->oxygen_dust : null,
                'ups' => $request->office_type == '83' ? $request->ups : null,
                // 'fire_safety_masures' => $request->fire_safety_masures,
                'lifts' => $request->office_type == '83' ? $request->lift : null,
        ...
        if ($save) {
            if ($request->fire_safety_masure) {
                foreach ($request->fire_safety_masure as $fire_saftey_masure) {
                    $save_fire_saftey_masure = FireSafteyMeasureMap::updateOrCreate(
                        [
                            'unit_id' => $request->unit_id,
                            'child_id' => $fire_saftey_masure,
                        ],
                        [
                            // 'status' => '1',
                        ],
                ...
                $remove = FireSafteyMeasureMap::where('unit_id', $request->unit_id)
                    ->whereNotIn('child_id', $request->fire_safety_masure)
                    ->delete();
            }

            if ($request->has('fire_safety_masure')) {
                $delete = UnitAmenityOptionValue::where('property_id', $request->property_id)
                    ->where('unit_id', $request->unit_id)
                    ->where('amenity_id', '8')
                    ->delete();
                foreach ($request->fire_safety_masure as $amenity) {
                    $amenityValue = new UnitAmenityOptionValue();
                    $amenityValue->property_id = $request->property_id;
                    $amenityValue->unit_id = $request->unit_id;
                    $amenityValue->amenity_id = '8';
    ...
    public function storePricingDetails(Request $request)
    {
        ...
        $validator = Validator::make(
            $request->all(),
            [
                'pricing_details_for' => ['required_if:unit_type_id,==,1'],
            ...
            [
                'pricing_details_for.required_if' => 'The pricing details for field is required when unit type is Vacant.',
                'ownership.required_if' => 'ownership field is required for Sale',
                'expected_price.required_if' => 'Expected Price is Required',
                'rented_price.required_if' => 'Rented Price is Required',
                'price_per_sq_ft.required_if' => 'Price Per Sq.Ft is Required',
                'expected_rent.required_if' => 'Expected Rent is Required',
                'rented_date.required_if' => 'Rented Date is Required',
                'sold_date.required_if' => 'Sold Date is Required',
                'sold_price.required_if' => 'Sold Price is Required',
        ...
        if ($request->pricing_details_for == 1) {
            // pricing_details_for=1 for sale and pricing_details_for=2 for rent
            $save = SecondaryUnitLevelData::updateOrCreate(
                [
                    'property_id' => $request->property_id,
                    'unit_id' => $request->unit_id,
                ...
                [
                    ...
                    'notice_period' => null,
                    'sale_status' => $request->new_resale_unit,
                    'sqft_based_on' => isset($request->sqft_based_on) ? $request->sqft_based_on : 1,
                ],
            );
            ...
            $save = SecondaryUnitLevelData::updateOrCreate(
                ...
                [
                    ...
                    'sqft_based_on' => isset($request->sqft_based_on) ? $request->sqft_based_on : 0,
            ...
            $save = SecondaryUnitLevelData::updateOrCreate(
                ...
                [
                    'pricing_details_for' => $request->pricing_details_for,
                    'agreement_type' => $request->agreement_type,
                    'expected_rent' => $request->rented_price,
                    // 'facility' => $request->facility,
                    'maintenance_rent' => $request->maintenance_rent,
                    'booking_amount_rent' => $request->booking_amount_rent,
                    'annual_dues_rent' => $request->annual_dues_rent,
            ...
            $save = SecondaryUnitLevelData::updateOrCreate(
                ...
                [
                    'pricing_details_for' => $request->pricing_details_for,
                    'ownership' => $request->ownership,
                    'expected_price' => $request->sold_price,
                    'rented_date' => $request->sold_date,
                    'price_per_sq_ft' => $request->price_per_sqft_sold,
                    // 'all_inclusive_price' => $request->all_inclusive_price,
                    // 'price_negociable' => $request->price_negociable,
                    'mainteinance' => $request->mainteinance,
                    'maintenance_period' => $request->price_period,
                    ...
                    'membership_charge' => $request->membership_charge,
                    'remark' => $request->remark,
                    'sqft_based_on' => isset($request->sqft_based_on_sold) ? $request->sqft_based_on_sold : 0,
                    // 'price_period' =>
                    'agreement_type' => null,
                    'expected_rent' => null,
                    // 'facility' => null,
    ...
    public function storeImages(Request $request)
    {
        ...
        $get_gallery_images_to_unlink = UnitImage::where('unit_id', $request->unit_id)
        ...
        $get_amenities_images_to_unlink = UnitImage::where('unit_id', $request->unit_id)
            ->where('file_type', 'amenities_images')
            ->get();
        // dd($get_images_to_unlink);
        ...
        $get_interior_images_to_unlink = UnitImage::where('unit_id', $request->unit_id)
        ...
        $get_floor_plan_images_to_unlink = UnitImage::where('unit_id', $request->unit_id)

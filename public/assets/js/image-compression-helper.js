...
// starts here...
    $(document).ready(function () {
        $("#imageInput").on("change", function (e) {
            ...
            (validStatus) ? handleImageCompression("#imageInput", e.target.files) : '';
        });
        $(document).on("change", "#image-gallery", function (e) {
            ...
            (validStatus) ? handleImageCompression("#image-gallery", e.target.files) : '';
        });
        $("#icm-amenities").on("change", function (e) {
            let validStatus = FFValidation({ 
                selector: '#icm-amenities',
                files : e.target.files,
                acceptedFiles:['.jpg', '.jpeg', '.png', '.gif'],
                maxFiles : 5
            });
            (validStatus) ? handleImageCompression("#icm-amenities", e.target.files): '';
        });
        $("#icm-interior").on("change", function (e) {
            let validStatus = FFValidation({ 
                selector: '#icm-interiorn',
                files : e.target.files,
                acceptedFiles:['.jpg', '.jpeg', '.png', '.gif','.pdf'],
                maxFiles : 5
            });
            (validStatus) ? handleImageCompression("#icm-interior", e.target.files): '';
        });
        $("#icm-floor-plan").on("change", function (e) {
            let validStatus = FFValidation({ 
                selector: '#icm-floor-plan',
                files : e.target.files,
                acceptedFiles:['.jpg', '.jpeg', '.png', '.gif'],
                maxFiles : 5
            });
            (validStatus) ? handleImageCompression("#icm-floor-plan", e.target.files): '';
        });
        // $(".icm-image").on("change", function (e) {
        //     let validStatus = FFValidation({ 
        //         selector: this,
        //         files : e.target.files,
        //         acceptedFiles:['.jpg', '.jpeg', '.png', '.gif'],
    ...
    $(document).on('click', '.icm-remove-img-item', function(){
        ...
        let deleteFileUrl = apiUrl + "/surveyor/property/unit-details/delete-unit-file/" + fileId;
        $.ajax({
            url: deleteFileUrl,
            type: "GET",
            dataType: "json",
                            ...
                            function (evt) {
                                if (evt.lengthComputable) {
                                    ...
                                    let currentPreviewEle =
                                        ...
                                        currentPreviewEle.css("width", percentComplete + "%");

                            ...
                            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="pills-compliances-tab" data-bs-toggle="pill"
                                        ...
                                        data-url="{{ route('compliances.index') }}">Compliances</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                                ...
                                                <div>
                                                    ...
                                                    <select class="form-select project-status" name="project_status"
                                                        ...
                                                        @forelse($project_status as $key=>$val)
                                                            <option value="{{ $val->id }}"
                                                                data-def_psddop_name="field-options-s0"
                                                                data-disabled_opt="{{ $val->options_self_validated }}"
                                                                data-level="0" class="disable-psof-{{ $val->id }}">
                                                                {{ $val->name }}</option>
                                                        @empty
                                                        @endforelse
                                                    </select>
...
@push('scripts')
    ...
    <script src="{{ asset('assets/js/property/gated/image-compression-helper.js') }}?v=5522358"></script>
    <script src="{{ asset('assets/js/property/gated/rera-hmda-image-compression-helper.js') }}?v=88229987755"></script>
    <script src="{{ asset('assets/js/property/gated/compliances.js') }}?v=78"></script>
    <script src="{{ asset('assets/js/property/gated/repository.js') }}?v=78"></script>
    <script>
    ...
    <script>
        ...
        function disableTowerStatusOptions(towerId) {
            ...
            $.ajax({
                ...
                url: "{{ url('surveyor/property/gated-community-details/tower-status/disabled-options') }}",
                success: function(response) {
                    // disabled_options
        ...
        $(document).on('submit', '#project-status-frm', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // $(project_status_options[0]).prop("selected", true);
                    // // Loop through each option starting from the second one (index 1)
                    // for (var i = 1; i < project_status_options.length; i++) {
                    // // Remove the selected property from the option
                    // $(project_status_options[i]).prop("selected", false);
                    // }
                    toggleLoadingAnimation();
                },
                error: function(xhr) {
        ...
        $(document).on('submit', '#edit-price-trends-frm', function(e) {
            ...
            toggleLoadingAnimation();
            $.ajax({
                url: "{{ url('surveyor/update-price-trends') }}",
                type: "POST",
                data: $(this).serialize() + '&property_id=' + propertyId,
                success: function(response) {
                    toastr.success(response.message);
    ...
    <!-- get tower name-->
    <script>
        // add units to all blocks on single click
        $(document).on("click", ".add-units-to-all-blocks", function() {
            toggleLoadingAnimation();
            let no_of_all_units = $('.no_of_all_units').val();
            $('.add-tower').each(function(i, v) {
                $('.no_of_towers' + i).val(no_of_all_units);
                $(this).trigger('click');
            });
    ...
    <script>
        ...
        $(document).on('click', '#pills-property-status-tab', function(e) {
            ...
            $.ajax({
                ...
                url: "{{ url('surveyor/property/gated-community-details/project-status/disabled-options') }}",
                success: function(response) {
                    // disabled_options
                    if (response.disabled_options != undefined) {
                        let disabledOptions = response.disabled_options;

...
@section('content')
@push('css')
<link href="{{ asset('assets/css/property-primary-details.css') }}?v=2553" rel="stylesheet" type="text/css" />
<link href="{{ asset('assets/css/autocompleteDropdown.css') }}?v=55885" rel="stylesheet" type="text/css" />
...
<div class="page-content">
                    ...
                    <div class="col-md-12 mt-2">
                        ...
                        <form class="unit-gallery-zone icm-zone ">
                            ...
                            <div class="row old-files-icm-lable-preview-group">
                                ...
                                @php $default_img = asset('assets/images/svg/default-pdf.svg') @endphp
                                <div class="dropzone-img-gallery col-md-6 ">
                                    @forelse($property->images as $image)
                                    <a data-fancybox="amenity_images" class="dropzone-img-gallery-item @if($loop->iteration > 3) d-none @endif" href="{{ asset('uploads/property/images/' . $image->file_url) }}">
                                        <img src="{{ asset('uploads/property/images/' . $image->file_url) }}" onerror="this.onerror=null; this.src='{{ $default_img }}'" class="img-fluid ri-pimg">
                                    </a>
                                    @empty
                                    @endforelse
                                </div>
    ...
    <!-- <script src="{{ asset('assets/js/property/add-images.js') }}?v=34567897654"></script> -->
    <script src="{{ asset('assets/js/image-compression-helper.js') }}?v=55888"></script>
    <script src="{{ asset('assets/js/property/split-merge.js') }}?v=6543"></script>
    <script src="{{ asset('assets/js/property/get-builder-sub-groups.js') }}?v=4"></script>
    <script src="{{ asset('assets/js/property/primaryDetailsInputHelper.js') }}?v=5558"></script>
    <script>
    ...
    <script>
        ...
        $(document).on('change', '.get-category-fields', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    ($('#update_property input[name="city"]').is(':visible')) ? fetchCity(): '';
                    autoCompleteConstructionPartnerSearch('.construction-partner-select-suggestions');
                    autoCompleteAreaSearch('.area-select-suggestions');
                }
            });
        ...
        $(document).on('click', ".add-unit", function() {
            ...
            if (count > 1 || currentStoreyUnitCount > 1) {
                ...
                $(totalFloors).insertAfter($(this).closest('.floor-dds_row').nextAll('.unit-container').first().find('.storey-unit').last()): $(this).closest('.floor-dds_row').nextAll('.unit-container').first().html(totalFloors);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if ($('#merge-gis').prop('checked') === true || $('#split-gis').prop('checked') === true) {
                        ...
                        setSplitMergeOptionDataAttributes();
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        let egis_ids = xhr.responseJSON.egis_ids;
                        console.log(egis_ids)
                        $('#update_property input[name="mgis_id[]"]').each(function() {
                            (egis_ids.includes(this.value)) ?
                            ($(this).addClass('is-invalid'),
                                $('<span class="input-error flash-errors" style="color: red">' + xhr.responseJSON.msg + '</span>')
        ...
        function setSplitMergeOptionDataAttributes() {
            let GISID = "{{$property->gis_id}}";
            $.ajax({
                type: "GET",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        function validateForm() {
            ...
            if ($('#merge-gis').prop('checked') === true) {
                mergeGISIDValuesValidStatus = areMergeGISIDValuesValid('#update_property input[name="mgis_id[]"]', 'property already exists with this Gis-ID.');
                let match_merge_ids = [];
            ...
            isValid = (isValid == false || floorNamesUniqueStatus == false || unitNamesUniqueStatus == false ||
                ...
                floorsStatus == false || mergeGISIDValuesValidStatus == false || matchGISMGISStatus == false) ? false : true;
            // return isValid;
        ...
        function areMergeGISIDValuesValid(selector, msg) {
            ...
            let priorGisId = "{{$property->gis_id}}";
            $.ajax({
                type: "GET",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')

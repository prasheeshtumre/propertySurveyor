                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    {{-- <div class="col-xxl-3 col-md-3 mt-3"> --}}
                                    <div class="col-xxl-3 col-md-3 mt-3 ">
                                        <label for="" class="form-label "> Min Area </label>
                                        <select class="form-select filter_dropdown" name="min_area" id="min_area">
                                    ...
                                    <div class="col-xxl-3 col-md-3 mt-3">
                                        <label for="" class="form-label "> Max Area </label>
                                        <select class="form-select filter_dropdown" name="max_area" id="max_area" disabled>
                                    ...
                                    <div class="col-xxl-3 col-md-3 mt-3 filters-hide" id="sub-categories">
                                        <div>
                                            <label for="" class="form-label ">Commercial Sub-Type</label>
                                            <select class="form-select filter_dropdown" name="subCategories" id="subCategories">
                                                <option value="">-Select Commercial Sub-Type-</option>
                                                @forelse($subCategories as $key=>$subCategories)
                                                    
                                                    <option value="{{ $subCategories->id }}">{{ $subCategories->cat_name }}</option>
                                                @empty
                                                @endforelse
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-xxl-3 col-md-3 mt-3 filters-hide "id="area-filter" hidden>
                                        <label for="" class="form-label ">Area </label>
                                        <div class="choose-suggestion-label-body">
                                            <input type="text" class="form-control input-select-suggestions area-select-suggestions" id="" placeholder="Choose Option">
                                            <label for="" class="choose-suggestion-label">Choose Option</label>
                                        </div>
                                    </div>

                                    <div class="col-xxl-3 col-md-3 mt-3 filters-hide" id="construction-partner-filter">
                                        <div>
                                            <label for="" class="form-label ">Construction Partner</label>
                                            <select class="form-select filter_dropdown select2-dd" name="construction_partner" id="construction_partner">
                                                <option value="">-Select Construction partner-</option>
                                                @forelse($constructionPartner as $key=>$constructionPartner)
                                                    <option value="{{ $constructionPartner->id }}">{{ strtoupper($constructionPartner->name) }}</option>
                                                @empty
                                                @endforelse
                                            </select>
                                        </div>
                                ...
                                <div class="col-md-12 col-lg-8">
                                    ...
                                    <a class="btn btn-secondary btn-sm ms-1 "
                                        href="{{ url('surveyor/property/excel/export') }}"><i
                                            class="fa-solid fa-file-excel me-1"></i>
                                        Excel</a>
                                    <a class="btn btn-primary btn-sm ms-1 cmd"> <i
                                            class="fa-regular fa-file-pdf me-1"></i> PDF</a>
...
@push('scripts')
        ...
        <script>
            ...
            $('#min_area').on('change', function () {
                ...
                $('#max_area option').each(function () {
                    if ($(this).val() !== '' && parseFloat($(this).val()) < parseFloat(selectedMinArea)) {
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '#filter-reset', function() {
            toggleLoadingAnimation();
            var url = "{{ route('surveyor.filter-data', $type) }}";
        ...
        $(document).ready(function() {
            ...
            function resedential() {
                ...
                $('#brand-cat-blk,#brand-sub-cat-blk,#brand-blk,#project-blk,#building-blk,#builder-name-blk,#owner-blk,#prop-type-blk,#boards-blk,#plot-lands-blk,#plot-name-blk,#const-type-blk')
                    .hide();
                if (selectValue2 == 10 || selectValue2 == 12 || selectValue1 == 8 || selectValue2 ==17 || selectValue2 ==9) {
                    if(selectValue1 == 8 && selectValue2 == 11 || selectValue2 ==9 ){
                        $('#builder-name-blk').show();
                        $('#unit-type-blk,#sub-categories').hide();
                    }else{
            ...
            function handleDropdownChange() {
                ...
                if (selectedValue == '5' && selectValue4 == '2' && selectValue1 !== '' && selectValue2 !== '') {
                    resedential();
                    $("#const-type-blk").show();
                }if(selectValue1 == '7' && selectValue2 == '11'){
                    $('#unit-type-blk,#sub-categories').hide();
                }
            }
            $('#subCategories').change(function() {
                ...
                if (selectedValue == '16') {
                    $('#area-filter').hide();
                    $('#construction-partner-filter').show();
                }
                if (selectedValue == '15') {
                    $('#area-filter').hide();
                    $('#construction-partner-filter').hide();
                }
            });
            $('#category').change(function() {
                ...
                $('.filters-hide').show();
                if (selectedValue == '1') {
                    $("#res-sub-type-blk,#project-blk,#res-type-blk,#prop-type-blk,#boards-blk,#plot-lands-blk,#plot-name-blk,#const-type-blk,#builder-name-blk,#construction-partner-filter")
                        .hide();
                }
                if (selectedValue == '2') {
                    $('.filters-hide').hide();
    ...
    <script>
        $(document).on('change', '#fltr_brand_category', function(e) {
            let c_id = $(this).val();
            $.ajax({
                type: "GET",
                data: {
                ...
                url: "{{ route('ajax.get-brand-sub-category') }}",
                success: function(response) {
                    $('#fltr_brand_sub_category').empty();
                    $('#fltr_brand_sub_category').append(
                        "<option value=''>-Select sub category-</option>");
                    if (response) {
                        $.each(response, function(key, value) {
                            $('#fltr_brand_sub_category').append($("<option/>", {
                                value: value.id,
                                text: value.name
                            }))
                        })
        ...
        $(document).on('change', '#fltr_brand_sub_category', function(e) {
            let c_id = $(this).val();
            $.ajax({
                type: "GET",
                data: {
                ...
                success: function(response) {
                    $('#fltr_brand_id').empty();
                    $('#fltr_brand_id').append(
                        "<option value=''>-Select Brand-</option>");
                    if (response) {
                        $.each(response, function(key, value) {
                            $('#fltr_brand_id').append($("<option/>", {
                                value: value.id,
                                text: value.name
                            }))
                        })
        ...
        $(document).on('change', '#fltr_residential_category', function(e) {
            let c_id = $(this).val();
            $.ajax({
                type: "GET",
                data: {
                ...
                url: "{{ route('ajax.get-sub-residential') }}",
                success: function(response) {
                    $('#fltr_residential_sub_category').empty();
                    $('#fltr_residential_sub_category').append(
                        "<option value=''>-Select Residential Sub Type-</option>");
                    if (response) {
                        $.each(response, function(key, value) {
                            $('#fltr_residential_sub_category').append($("<option/>", {
                                value: value.id,
                                text: value.cat_name
                            }))
                        })
